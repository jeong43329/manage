...
<div id="container">
 ...
 <script type="text/javascript">
	$("#btn検索").click(function(){
		$.ajax({
			...
			success: function(data){//success는 콜백함수 서버에서 데이터를 정상적으로 응답받으면 실행된다
				$("#container").html(data);//container안의 내요을 업데이트(data)
			},
			error: function(){
	...
	$(document).on('click',"[id='previousPage']",function(){
		event.preventDefault();//버튼의 디폴트인 submit이벤트를 무효한다
		var selecting=$('#currentPage').val();//콤보박스에서 선택된 페이지번호를 취득

		if(selecting==1){//만약 현페이지가 1페이지이면 다음처리에 가지않는다.
			return;
		}

		$('#currentPage').val(Number(selecting)-1);//콤보박스의 표시페이지번호에서 1을뺸다
		$('#nextPage').attr("src","${pageContext.request.contextPath}/img/right_triangle.png");//유효한 버튼의 이미지를 설정한다
